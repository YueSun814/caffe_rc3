...
void HDF5DataLayer<Dtype>::LoadHDF5FileData(const char* filename) {
  ...
  int top_size = this->layer_param_.top_size();//the number of top outputs, e.g., 2 or 3, data/label
  ...
  for (int i = 0; i < top_size; ++i) {
    ...
    hdf5_load_nd_dataset(file_id, this->layer_param_.top(i).c_str(),//thus, here, we need the name of top blob to be the same of which inside the hdf5 dataset
...
void HDF5DataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (source_file.is_open()) {
    ...
    while (source_file >> line) {
      hdf_filenames_.push_back(line);//push the hdf5 file names into this vector
    }
  } else {
  ...
  for (int i = 0; i < batch_size; ++i, ++current_row_) {
    if (current_row_ == hdf_blobs_[0]->shape(0)) {
      if (num_files_ > 1) {
        ...
        LoadHDF5FileData(
            hdf_filenames_[file_permutation_[current_file_]].c_str());//load hdf5 data (a hdf5 file from the hdf5 file lists
      }
      current_row_ = 0;
      if (this->layer_param_.hdf5_data_param().shuffle())
        std::random_shuffle(data_permutation_.begin(), data_permutation_.end());
